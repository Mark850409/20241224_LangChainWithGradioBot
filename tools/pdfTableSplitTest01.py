...
def image_qa(query, image_path, prompt=None):
...
class NewVectorDBConnector:
    ...
    def add_images(self, image_paths):
        documents = [
            image_qa("请简要描述图片中的信息",image,prompt="請一律使用繁體中文回覆!!!")
...
if __name__ == "__main__":
    ...
    output_directory_path = pdf2images("../input/1130123家用Wi-Fi機件性能與寬頻傳輸速率供裝對應表.pdf")
    ...
    # 3 - 重複提問
    while True:
        query = input("請輸入您的問題 (輸入 '退出' 結束程序): ")
       ...
       # 執行查詢，指定返回的結果數量
        try:
            results = new_db_connector.search(query, top_n=5)
            metadata = results["metadatas"][0]
            logger.info(f"最後得到的檢索結果是：{metadata}")
            response = image_qa(query, metadata[0]["image"])
            logger.info(f"最後得到的回答是：{response}")
            print(f"AI回答：{response}")
        except Exception as e:
            logger.error(f"查詢過程中出現錯誤: {str(e)}")
            print(f"發生錯誤：{str(e)}")